...
<div class="card bg-base-100 border">
    <div class="card-body">
        <form action="<?= base_url('dashboard/master_prodi/update/' . $prodi->id) ?>" method="post">
            <div class="grid grid-cols-12 grid-flow-row gap-4">
                    ...
                    <select name="kode_fakultas" id="kode_fakultas" class="select select-bordered w-full">
                        <?php if (count($fakultas) > 0): ?>
                            <?php foreach($fakultas as $fak): ?>
                                <option <?= $fak->kode_fakultas == $prodi->kode_fakultas ? "selected" : "" ?> value="<?= $fak->kode_fakultas ?>">
                ...
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    ...
                    <input type="text" name="kode_prodi" id="kode_prodi" class="input input-bordered w-full" value="<?= $prodi->kode_prodi ?>" />
                </div>
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    <label for="nama" class="label">
                        <span class="label-text">Nama</span>
                    </label>
                    <input type="text" name="nama" id="nama" class="input input-bordered w-full" value="<?= $prodi->nama ?>" />
                </div>
