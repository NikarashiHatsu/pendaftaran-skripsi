...
<div class="card bg-base-100 border">
    <div class="card-body">
        <form action="<?= base_url('dashboard/sidang/update/' . $sidang->id) ?>" method="post" enctype="multipart/form-data">
            <div class="grid grid-cols-12 grid-flow-row gap-4">
                ...
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    ...
                    <input type="date" name="tanggal_sidang" id="tanggal_sidang" class="input input-bordered w-full" value="<?= $sidang->tanggal_sidang ?>" required />
                </div>
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    ...
                    <select name="nip_penguji1" id="nip_penguji1" class="select select-bordered w-full" required>
                        <?php if (count($dosens) > 0): ?>
                            <?php foreach($dosens as $dosen): ?>
                                <option <?=  $dosen->nip == $sidang->nip_penguji1 ? "selected" : "" ?> value="<?= $dosen->nip ?>">
                ...
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    ...
                    <select name="nip_penguji2" id="nip_penguji2" class="select select-bordered w-full" required>
                        <?php if (count($dosens) > 0): ?>
                            <?php foreach($dosens as $dosen): ?>
                                <option <?=  $dosen->nip == $sidang->nip_penguji2 ? "selected" : "" ?> value="<?= $dosen->nip ?>">
                                    (<?= $dosen->nip ?>) <?= $dosen->nama ?>
                                </option>
                            <?php endforeach; ?>
                        <?php else: ?>
                ...
                <div class="col-span-12 sm:col-span-6 md:col-span-4">
                    ...
                    <select name="ruangan" id="ruangan" class="select select-bordered w-full" required>
                        <option <?= $sidang->ruangan == "rs1" ? "selected" : "" ?> value="rs1">Ruangan 1</option>
                        <option <?= $sidang->ruangan == "rs2" ? "selected" : "" ?> value="rs2">Ruangan 2</option>
                        <option <?= $sidang->ruangan == "rs3" ? "selected" : "" ?> value="rs3">Ruangan 3</option>
                        <option <?= $sidang->ruangan == "rs4" ? "selected" : "" ?> value="rs4">Ruangan 4</option>
                    </select>
                </div>
